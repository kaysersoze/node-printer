var printer_helper = {},
    ...
    os = require("os"),
    path = require("path"),
    binding_path = path.join(__dirname, './binding/node_printer.node'),
    precompiled_path = path.join(__dirname, './precompiled/node_printer_' +
    	process.versions.modules + '_' + process.platform + '_' + process.arch + '.node')
    printer_helper;

if(fs.existsSync(precompiled_path)) {
	console.log('PRECOMPILED', precompiled_path);
    printer_helper = require(precompiled_path);
} else {
	console.log('DEFAULT', binding_path);
